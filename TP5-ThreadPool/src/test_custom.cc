...
bool test_potential_deadlock() {
    ...
    thread t([&]() {
        try {
            ...
            pool.schedule([&]() {
                // Al adquirir mtx, actualizamos ready bajo protección adicional
                lock_guard<mutex> lock(mtx);
                {
            ...
            // Si no se pudo adquirir mtx y la tarea no marcó ready,
            // se infiere un posible deadlock y se notifica la finalización.
