...
int32_t decode_branch_offset(uint32_t instruction) {
    ...
    int32_t offset = decode_extract_bits(instruction, 0, 25);
...
uint32_t decode_condition(uint32_t instruction) {
    return decode_extract_bits(instruction, 0, 3);
}